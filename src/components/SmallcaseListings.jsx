...
const SmallcaseListings = () => {
  ...
  let investmentStrategies = smallcases.reduce((acc, curr) => {
    ...
    if (!investmentStrategyArray) return acc;
  ...
  const filterByVolatility = (smallcase, volatilitySet) => {
    if (volatilitySet.size === 0) return true;
    let currVolatility = smallcase.stats.ratios.riskLabel;
    if (volatilitySet.has(currVolatility)) return true;
  };
  ...
  const filterByInvestmentStrategies = (smallcase, investmentStrategiesSet) => {
    if (investmentStrategiesSet.size === 0) return true;

    let investmentStrategyArray = smallcase.info.investmentStrategy;

    for (let strategy of investmentStrategyArray) {
      if (investmentStrategiesSet.has(strategy.displayName)) return true;
    }
    return false;
  };

  const sortByPopularity = (smallcases) => {
    return smallcases.sort(
      (a, b) =>
  ...
  const sortedData = (smallcases) => {
    switch (selectedSort.sortBy) {
      ...
      case "1M":
        return sortByReturns(
          smallcases,
          "monthly",
          selectedSort.orderBy.order === "High-Low" ? "desc" : "asc"
        );
      case "6M":
        return sortByReturns(
          smallcases,
  ...
  const filteredSmallCases = smallcases
    .filter((smallcase) =>
      filterBySubscriptionType(smallcase, filters.subscriptionType)
    )
    .filter((smallcase) =>
      filterByInvestmentAmount(smallcase, filters.investmentAmount)
    )
    ...
    .filter((smallcase) =>
      filterByInvestmentStrategies(smallcase, filters.investmentStrategiesSet)
    );

  const sortedSmallCases = sortedData(filteredSmallCases);

  const renderSmallCases = sortedSmallCases.map((smallCase) => {
    return (
      <Card
        key={smallCase._id}
        smallCaseInfo={smallCase}
    ...
    <div>
      ...
      <main className="main flex gap-x-12 justify-center mt-2">
        ...
        {renderSmallCases.length > 0 ? (
          <ul className=" w-[900px]">{renderSmallCases}</ul>
        ) : (
          <Notfound />
        )}
      </main>

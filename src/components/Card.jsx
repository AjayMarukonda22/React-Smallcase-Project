...
const Card = ({ smallCaseInfo, selectedSort }) => {
  ...
  if (selectedSort.orderBy.status) {
    ...
    if (returnPeriod) {
      cagrDuration = `${returnsType} ${
        returnsType.includes("M") ? "Returns" : "CAGR"
      }`;
      ...
      <div className="flex gap-x-8 justify-start w-[50%]">
        ...
        <div>
          <div className="flex gap-x-2 justify-start items-center">
            ...
            <p className="bg-gray-100 px-1 py-0.5 text-[0.65rem] font-bold rounded-xs text-blue-500 ml-2">
              {!subscriptionType ? "Free Access" : ""}
            </p>
          </div>
          <p className="py-1 text-gray-700 text-[0.98rem]">
            {shortDescription}
          </p>
